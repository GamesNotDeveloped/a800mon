...
func (c *Client) Sysinfo(ctx context.Context) (Sysinfo, error) {
	data, err := c.Call(ctx, CmdSysinfo, nil)
	if err != nil {
		return Sysinfo{}, err
	}
	if len(data) < 5 {
		return Sysinfo{}, errors.New("sysinfo payload too short")
	}
	flags := data[0]
	return Sysinfo{
		BasicEnabled:        flags&0x01 != 0,
		TVPAL:               flags&0x02 != 0,
		MachineFamily:       data[1],
		OSRevision:          data[2],
		BasicRevision:       data[3],
		BuiltinGameRevision: data[4],
	}, nil
}
