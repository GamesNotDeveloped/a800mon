...
class RpcClient:
    ...
    async def sysinfo(self):
        data = await self.call(Command.SYSINFO)
        if len(data) < 5:
            raise RpcException("SYSINFO payload too short")
        (
            flags,
            machine_family,
            os_revision,
            basic_revision,
            builtin_game_revision,
        ) = struct.unpack("<BBBBB", data[:5])
        return Sysinfo(
            machine_family=machine_family,
            os_revision=os_revision,
            basic_revision=basic_revision,
            builtin_game_revision=builtin_game_revision,
            basic_enabled=bool(flags & 0x01),
            tv_pal=bool(flags & 0x02),
        )
